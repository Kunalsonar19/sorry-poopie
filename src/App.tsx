...
const InitialCard: React.FC<{ onNext: () => void }> = ({ onNext }) => {
  const messages = [
    "Heyy Pooopie 💩",
    "Good Morning 🌻",
    "I have so much to say, but before that...",
    "It's been a while we haven't played a game!",
    "So.......",
    "Let's play a game!",
  ];
  ...
  return (
    <div
      className={`relative flex items-center justify-center transition-opacity duration-500 ${
        componentFade === "in" ? "opacity-100" : "opacity-0"
      }`}
    >
      <div className="bg-gradient-to-br from-red-500 to-red-700 rounded-3xl shadow-2xl p-8 w-80 h-96 transform transition-all duration-500 hover:scale-105">
        ...
        <div className="bg-white rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-6 border-4 border-white shadow-inner">
          <span className="text-red-600 font-bold text-3xl">VS</span>
        </div>

        {/* Animated Message */}
        <div className="h-32 flex items-center justify-center">
          <h1
            className={`text-white text-2xl font-bold text-center transition-opacity duration-1000 ${
...
const MusicCard: React.FC<{
  ...
  return (
    <div
      className={`relative perspective-1000 transition-opacity duration-1000 m-4 ${
    ...
    >
      <div className="bg-blue-600 rounded-3xl shadow-2xl p-8 transform transition-all duration-500 hover:scale-105 max-w-md mx-auto">
        ...
        <div className="bg-white rounded-full w-24 h-24 flex items-center justify-center mx-auto mb-6 border-8 border-white shadow-inner">
          <span className="text-blue-600 font-bold text-4xl">VS</span>
        </div>

        <h2 className="text-white text-3xl font-bold text-center mb-6">
          Whenever I look into your eyes, this plays.
        </h2>

        <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-white font-bold text-xl">Ankhon Mein Teri</h3>
        ...
        <div className="mt-8 text-center">
          ...
          >
            More From Heart <ChevronRight className="ml-2" />
          </button>
        </div>
      </div>
    </div>
...
function App() {
  const [currentSection, setCurrentSection] = useState<
    "initial" | "music" | "cards" | "gallery" | "final"
  >("initial");
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full">
        ...
        {currentSection === "gallery" && (
          <PhotoGallery
            onNext={() => handleSectionChange("final")}
            onContinue={() => handleSectionChange("final")}
          />
        )}
